...
function tagged_mt:__tocbor(opts)
	return integer(self.tag, 192) .. encode(self.value, opts)
end
...
function encoder.float(num)
	...
	return s_char(
		...
		exponent % 2 ^ 4 * 2 ^ 4 + m_floor(fraction * 2 ^ 4 % 0x100),
...
if s_unpack then
	function read_float(fh)
		return s_unpack(">f", read_bytes(fh, 4))
	end
	function read_double(fh)
		return s_unpack(">d", read_bytes(fh, 8))
	end
end
